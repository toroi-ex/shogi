...
final2::final2(RGBImage image,RGBImage image_kai, RGBImage image_ras, RGBImage image1, RGBImage image2, RGBImage image3, RGBImage image4, RGBImage image5) : size_x(image.sizeX), size_y(image.sizeY),
...
size_x3(image3.sizeX), size_y3(image3.sizeY), size_x4(image4.sizeX), size_y4(image4.sizeY), size_x5(image5.sizeX), size_y5(image5.sizeY) {
	...
	data = new struct color*[size_y];  //前の画像
	for (int i = 0; i < size_y; i++) {
		data[i] = new struct color[size_x];
	}

	data_kai = new struct color*[size_y];  //後の画像
	for (int i = 0; i < size_y; i++) {
		data_kai[i] = new struct color[size_x];
	}

	data_op = new struct color*[size_y];  //後の画像
	for (int i = 0; i < size_y; i++) {
		data_op[i] = new struct color[size_x];
	}
	...
	for (int y = 0; y < size_y; y++) {						//白消すやーつと思ったが復元するやつ
		for (int x = 0; x < size_x; x++) {
			data_kai[y][x].red = image_kai.data[y][x].r;
			data_kai[y][x].green = image_kai.data[y][x].g;
			data_kai[y][x].blue = image_kai.data[y][x].b;
...
//歩
int final2::SAD1(int y, int x) {

	if (x + size_x1 - 1 < size_x && y + size_y1 - 1 < size_y) {
		int ssd = 0;
		for (int k = 0; k < size_y1; k++) {
			for (int l = 0; l < size_x1; l++) {
				int tmp = (int)(data_op[k + y][l + x].red - templete_data1[k][l].red);
				if (tmp < 0) {
					tmp *= -1;
				}
				ssd += tmp;
...
//柱
int final2::SAD2(int y, int x) {

	if (x + size_x2 - 1 < size_x && y + size_y2 - 1 < size_y) {
		int ssd = 0;
		for (int k = 0; k < size_y2; k++) {
			for (int l = 0; l < size_x2; l++) {
				int tmp = (int)(data_op[k + y][l + x].red - templete_data2[k][l].red);
				if (tmp < 0) {
					tmp *= -1;
				}
				ssd += tmp;
...
//香
int final2::SAD3(int y, int x) {

	if (x + size_x3 - 1 < size_x && y + size_y3 - 1 < size_y) {
		int ssd = 0;
		for (int k = 0; k < size_y3; k++) {
			for (int l = 0; l < size_x3; l++) {
				int tmp = (int)(data_op[k + y][l + x].red - templete_data3[k][l].red);
				if (tmp < 0) {
					tmp *= -1;
				}
				ssd += tmp;
...
int final2::white_detect(int y, int x, int size_y_tmp, int size_x_tmp) {
	int sum = 0;
	for (int i = y; i < y + size_y_tmp; i++) {
		for (int j = x; j < x + size_x_tmp; j++) {
			if (data_op[i][j].red == 255) {
				sum++;
			}
		}
	}
...
void final2::do_match() {
	...
	for (int y = 0; y < size_y; y++) {										//先に動いたほうを検出
		for (int x = 0; x < size_x; x++) {
			data_op[y][x].red = data_kai[y][x].red;
			data_op[y][x].green = data_kai[y][x].green;
			data_op[y][x].blue = data_kai[y][x].blue;
		}
	}
	...
	for (int y = 0; y < size_y; y++) {										//動く前を検出
		for (int x = 0; x < size_x; x++) {
			data_op[y][x].red = data[y][x].red;
			data_op[y][x].green = data[y][x].green;
			data_op[y][x].blue = data[y][x].blue;
		}
	}
...
//駒の位置特定
int final2::specify(int point_x, int point_y) {
	int a, b;	//ループ変数
...
void final2::output_RGBImage(RGBImage *image_print) {
